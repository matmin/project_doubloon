...
class TradeRepublicCSVProvider:
    name = "trade_republic_csv"
    ...
    def parse(self, file_obj) -> Iterable[Dict[str, Any]]:
        ...
        for raw_row in reader:
            ...
            tx_date = parse_date_flex(
                self._get(row, "date"), ["%Y-%m-%d", "%d/%m/%Y", "%d.%m.%Y"]
            )
            description = self._get(row, "description", "name") or ""
            asset_type = (self._get(row, "assettype", "asset_type") or "").lower()
            tx_type_raw = (self._get(row, "type") or "").lower()
            raw_amount = self._get(row, "amount", "total")
            amount = parse_amount_smart(raw_amount) if raw_amount else None
            isin = self._get(row, "isin")
            shares = parse_amount_smart(self._get(row, "shares") or "")
            price = parse_amount_smart(self._get(row, "price") or "")
            fee = parse_amount_smart(self._get(row, "fee") or "") or 0.0
            tax = parse_amount_smart(self._get(row, "tax") or "") or 0.0
            ...
            if "deposit" in tx_type_raw or "payment in" in tx_type_raw:
